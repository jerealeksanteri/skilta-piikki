...
import { getRouletteAdminStats, toggleRoulette } from '../api/roulette';
import type { RouletteAdminStats } from '../api/roulette';
...
export default function AdminRoulettePage() {
  ...
  const [stats, setStats] = useState<RouletteAdminStats | null>(null);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const data = await getRouletteAdminStats(scope);
      setStats(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleToggle = async () => {
    ...
    if (!confirm(`Are you sure you want to ${action} roulette?`)) return;

    setToggling(true);
    try {
      await toggleRoulette(newEnabled);
      setStats({ ...stats, enabled: newEnabled });
      showToast(`Roulette ${newEnabled ? 'enabled' : 'disabled'}`);
    } catch (e) {
    ...
    <div>
      ...
      <div style={styles.section}>
        ...
        <button
          ...
          disabled={toggling}
        >
          {stats.enabled ? 'Disable Roulette' : 'Enable Roulette'}
        </button>
      </div>
